...
class Replay(object):
    def __init__(self, s_screen, s_info, alive, alive_, act,
                 r, s__screen, s__info):
        ...
        self.s__info = s__info

    def get_s_screen(self):
        return self.s_screen
...
class Memory(object):
    ...
    def store_replay(self, s_screen, s_info, alive, alive_, a, r, s__screen, s__info):
        # 将每一step的经验加入经验池
        # 经验池未满时正常加入经验
        replay = Replay(s_screen, s_info, alive, alive_, a, r, s__screen, s__info)
    ...
    def random_sample_replay(self, batch_size, gpu):
    ...
    def sample_replay(self, indexes):
        ...
        alive_batch = torch.FloatTensor(np.array(alive_batch)).cuda()
        alive__batch = torch.FloatTensor(np.array(alive__batch)).cuda()
        a_batch = torch.FloatTensor(np.array(a_batch)).cuda()
        r_batch = torch.FloatTensor(np.array(r_batch)).cuda()
        s__screen_batch = torch.FloatTensor(np.array(s__screen_batch)).cuda()
        s__info_batch = torch.FloatTensor(np.array(s__info_batch)).cuda()
        alive_batch = alive_batch.view(-1, 10, 1)
        alive__batch = alive__batch.view(-1, 10, 1)
        r_batch = r_batch.view(-1, 10, 1)

        return [s_screen_batch, s_info_batch, alive_batch, alive__batch, a_batch,
                r_batch, s__screen_batch, s__info_batch]
