...
class NetDetectorCritic(nn.Module):
    # 输入img(screen)、info信息、所有Agent的action
    # 输出价值q_value

    def __init__(self, agent_num):
        super(NetDetectorCritic, self).__init__()
        self.conv = nn.Sequential(  # batch * 100 * 100 * 5
            nn.Conv2d(
                in_channels=5,
                out_channels=8,
...
class NetDetectorActor(nn.Module):
    # 决定航向与攻击类型的Actor网络
    # 输入img（screen）信息、info信息、agent存活信息
    # 输出航向（[0-359]离散值）、攻击类型(连续值)

    def __init__(self):
        super(NetDetectorActor, self).__init__()
        self.conv = nn.Sequential(  # batch * 100 * 100 * 3
            nn.Conv2d(
                in_channels=5,
                out_channels=8,
...
class RLFighter:
    def __init__(
        ...
        # 经验池
        self.memory_fig = Memory(max_memory_size)
        self.memory_det = Memory(max_memory_size)
        ...
        self.eval_net_critic_detector, self.target_net_critic_detector = NetDetectorCritic(self.agent_num), NetDetectorCritic(self.agent_num)
        self.eval_net_actor_detector, self.target_net_actor_detector = NetDetectorActor(), NetDetectorCritic()
        

        # 初始化网络参数
        for m1, m2 in zip(self.eval_net_actor_detector.modules(), self.eval_net_actor_fighter.modules()):
        ...
        self.copy_param(self.eval_net_critic_detector, self.target_net_critic_detector, 0)
        self.copy_param(self.eval_net_actor_detector, self.target_net_actor_detector, 0)
        ...
        if self.gpu_enable:
            ...
            self.eval_net_critic_detector = self.eval_net_critic_detector.cuda()
            self.target_net_critic_detector = self.target_net_critic_detector.cuda()
            self.eval_net_actor_detector = self.eval_net_actor_detector.cuda()
            self.target_net_actor_detector = self.target_net_actor_detector.cuda()
        ...
        self.critic_optimizer_detector = torch.optim.Adam(self.eval_net_critic_detector.parameters(), lr=self.cr_lr)
        self.actor_optimizer_detector = torch.optim.Adam(self.eval_net_actor_detector.parameters(), lr=self.ac_lr)
    ...
    def store_replay(self, s, alive, alive_, a, ma, oa, r, s_, type):
        # 将每一step的经验加入经验池
        if type == 'fighter':
            self.memory_fig.store_replay(s, alive, alive_, a, ma, oa, r, s_)
        else:
            self.memory_det.store_replay(s, alive, alive_, a, ma, oa, r, s_)

    def get_memory_size(self):
        return self.memory_fig.get_size()

    # 选择行为(只针对单个数据，不是batch，用于决策时)
    def choose_action(self, img_obs, info_obs, alive, type):
        ...
        if type == 'fighter':
            ...
            act = self.target_net_actor_detector(img_obs, info_obs, alive).cpu()
            course = torch.tanh(act[:, 0]) * 180
            try:
                # 航向
                course = int(course)
    ...
    def choose_action_batch(self, img_obs_batch, info_obs_batch, alive_batch, type):
        ...
        if type == 'fighter':
            ...
            radar = radar.unsqueeze(radar, 1)
            disturb = disturb.unsqueeze(radar, 1)
            attack = torch.unsqueeze(attack, 1)
            action = torch.cat([course, radar, disturb, attack], 1)
            return action
        else:
            act = self.eval_net_actor_detector(img_obs_batch, info_obs_batch, alive_batch)
            # 航向
            course = torch.tanh(act[:, 0]) * 180
            ...
            course = torch.unsqueeze(course, 1)
            radar = radar.unsqueeze(radar, 1)
            action = torch.cat([course, radar], 1)
            return action

    # 针对batch的数据选择行为(使用target_actor网络计算)
    def choose_action_batch_tar(self, img_obs_batch, info_obs_batch, alive_batch, type):
        noise = torch.randn(self.batch_size, 1) * self.noise_rate
        if self.gpu_enable:
            img_obs_batch = img_obs_batch.cuda()
            info_obs_batch = info_obs_batch.cuda()
        ...
        if type == 'fighter':
            ...
            act = self.target_net_actor_detector(img_obs_batch, info_obs_batch, alive_batch)
            # 航向
            course = torch.tanh(act[:, 0]) * 180
    ...
    def learn(self, save_path, writer):
        ...
        if self.learn_step_counter % self.replace_target_iter == 0:
            ...
            self.copy_param(self.eval_net_critic_detector, self.target_net_critic_detector, self.tau)
            self.copy_param(self.eval_net_actor_detector, self.target_net_actor_detector, self.tau)
            print('\ntarget_params_replaced\n')
            step_counter_str = '%09d' % self.learn_step_counter
            torch.save(self.target_net_critic_fighter.state_dict(),
                       save_path + '/fig/critic/model_' + step_counter_str + '.pkl')
            torch.save(self.target_net_actor_fighter.state_dict(),
                       save_path + '/fig/actor/model_' + step_counter_str + '.pkl')
            torch.save(self.target_net_critic_detector.state_dict(),
                       save_path + '/det/critic/model_' + step_counter_str + '.pkl')
            torch.save(self.target_net_actor_detector.state_dict(),
                       save_path + '/det/actor/model_' + step_counter_str + '.pkl')
            if self.pkl_counter < self.max_pkl:
                self.pkl_counter += 1
            else:
                # 删除最旧的模型参数
                fig_critic_path = os.path.join(save_path, 'fig/critic')
                fig_actor_path = os.path.join(save_path, 'fig/actor')
                det_critic_path = os.path.join(save_path, 'det/critic')
                det_actor_path = os.path.join(save_path, 'det/actor')             

                files = os.listdir(fig_critic_path)
                for file in files:
                    if file.endswith('pkl'):
                        os.remove(os.path.join(fig_critic_path, file))
                        break

                files = os.listdir(fig_actor_path)
                for file in files:
                    if file.endswith('pkl'):
                        os.remove(os.path.join(fig_actor_path, file))
                        break
                        
                files = os.listdir(det_critic_path)
                for file in files:
                    if file.endswith('pkl'):
                        os.remove(os.path.join(det_critic_path, file))
                        break

                files = os.listdir(det_actor_path)
                for file in files:
                    if file.endswith('pkl'):
                        os.remove(os.path.join(det_actor_path, file))
                        break

        # 采样Replay
        [s_screen_batch, s_info_batch, alive_batch, alive__batch, self_a_batch,
         mate_a_batch, other_a_batch, r_batch, s__screen_batch, s__info_batch] = self.memory_fig.sample_replay(
            self.batch_size, self.gpu_enable)

        [s_screen_batch_det, s_info_batch_det, alive_batch_det, alive__batch_det, self_a_batch_det,
         mate_a_batch_det, other_a_batch_det, r_batch_det, s__screen_batch_det, s__info_batch_det] = self.memory_det.sample_replay(
            self.batch_size, self.gpu_enable)
