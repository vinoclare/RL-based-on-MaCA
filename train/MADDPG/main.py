...
# 清除tensorboard文件
for file in os.listdir('D:/MaCA/runs/double'):
    path = os.path.join('D:/MaCA/runs/double', file)
    os.remove(path)
...
if __name__ == "__main__":
    ...
    blue_fighter_model = MADDPG.RLFighter(name='blue', agent_num=(DETECTOR_NUM + FIGHTER_NUM) * 2,
                                          attack_num=ATTACK_IND_NUM,
                                          fighter_num=FIGHTER_NUM)
    red_detector_action = []
    red_fighter_model = MADDPG.RLFighter(name='red', agent_num=(DETECTOR_NUM + FIGHTER_NUM) * 2,
                                         attack_num=ATTACK_IND_NUM,
                                         fighter_num=FIGHTER_NUM)
    ...
    writer = SummaryWriter('runs/double')
    ...
    for x in range(MAX_EPOCH):
        ...
        red_epoch_reward = 0  # 记录一个epoch内的红方平均reward
        while True:
            # 获取双方初始环境观测
            if step_cnt == 0:
            ...
            # 获取红色方行动
            red_alive = []  # 红队全队存活信息
            red_obs_list = []  # 红色方的全体环境观测信息
            if step_cnt <= 50 or step_cnt % 10 == 0:  # 10个step内航向保持一致，只更新攻击类型
                red_fighter_action = []  # 红色方所有agent的行动
            get_info(red_obs_dict, red_fighter_num, red_fighter_model, red_fighter_action,
                     red_obs_list, red_alive, step_cnt, 0)
            ...
            red_epoch_reward += np.mean(red_fighter_reward)
            ...
            for y in range(blue_fighter_num):
                if blue_alive[y]:
                    ...
                    blue_fighter_model.store_replay(blue_obs_list[y], blue_alive[y], alive_, self_action,
                                                    mate_fighter_action, red_fighter_action, blue_fighter_reward[y],
                                                    blue_obs_list_)
            ...
            for y in range(red_fighter_num):
                if red_alive[y]:
                    ...
                    alive_ = int(red_obs_dict['fighter'][y]['alive'])
                    self_action = red_fighter_action[y]
                    mate_fighter_action = np.concatenate((red_fighter_action[:y], red_fighter_action[y + 1:]), 0)
                    red_obs_list_ = {'screen': copy.deepcopy(tmp_img_obs), 'info': copy.deepcopy(tmp_info_obs)}
                    red_fighter_model.store_replay(red_obs_list[y], red_alive[y], alive_, self_action,
                                                   mate_fighter_action, blue_fighter_action, red_fighter_reward[y],
                                                   red_obs_list_)

            # 环境判定完成后（回合完毕），开始学习模型参数
            if env.get_done():
                # detector_model.learn()
                blue_fighter_model.learn('model/MADDPG/blue', writer)
                red_fighter_model.learn('model/MADDPG/red', writer)
