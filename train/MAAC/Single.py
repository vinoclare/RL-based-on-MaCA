...
from common.Replay3 import Memory as RedMemory
from train.MAAC import MAAC
...
start_learn_threshold = 256  # 当经验池积累5000条数据后才开始训练

# 清除tensorboard文件
for file in os.listdir('C:/MaCA/runs/MAAC/single'):
    path = os.path.join('C:/MaCA/runs/MAAC/single', file)
    os.remove(path)

if __name__ == "__main__":
    # 红色方为fix rule no attack，蓝色方为MAAC
    red_agent = Agent()
    ...
    RedMemory = RedMemory(2e4)
    blue_fighter_model = MAAC.RLFighter(name='blue', agent_num=(DETECTOR_NUM + FIGHTER_NUM), attack_num=ATTACK_IND_NUM,
                                        fighter_num=FIGHTER_NUM, radar_num=RADAR_NUM)
    red_agent.set_map_info(size_x, size_y, blue_detector_num, blue_fighter_num)

    global_step_cnt = 0
    writer = SummaryWriter('runs/MAAC/single')
        ...
        while True:
            ...
            if step_cnt % 50 == 0:  # 50个step内航向保持一致，只更新攻击类型
                ...
                for y in range(blue_fighter_num):  # 可以不用for循环，直接矩阵计算
                    ...
                    true_action = blue_fighter_model.choose_action(tmp_img_obs, tmp_info_obs, y)
                    blue_screen_list.append(copy.deepcopy(tmp_img_obs))
                    blue_info_list.append(copy.deepcopy(tmp_info_obs))
            ...
            # 红色方fix_rule_no_attack的动作样式转换为与MAAC一致
            red_fighter_action2 = []
            ...
            for y in range(blue_fighter_num):
                ...
                tmp_img_obs = tmp_img_obs.transpose(2, 0, 1)
                tmp_info_obs = blue_obs_dict['fighter'][y]['info']
                alive_.append(int(blue_obs_dict['fighter'][y]['alive']))
                blue_screen_list_.append(copy.deepcopy(tmp_img_obs))
                blue_info_list_.append(copy.deepcopy(tmp_info_obs))

            blue_fighter_model.store_replay(blue_screen_list, blue_info_list, blue_alive, alive_, blue_fighter_action,
                                            blue_fighter_reward, blue_screen_list_, blue_info_list_)

            RedMemory.store_replay(red_fighter_action2)

            # 环境判定完成后（回合完毕），开始学习模型参数
            if env.get_done():
                mem_size = blue_fighter_model.get_memory_size()
                batch_indexes = random.sample(range(mem_size), BATCH_SIZE)
                blue_fighter_model.learn('model/MAAC/single', writer=writer, batch_indexes=batch_indexes, red_mem=RedMemory)
