...
class NetFighterCritic(nn.Module):
    ...
    def __init__(self, agent_num):
        ...
        for i in range(agent_num):
            ...
            critic.add_module('fc1', nn.Linear(512 * 4 + 256, 256))
        ...
        for i in range(4):
            self.key_extractors.append(nn.Linear(256, 512, bias=False))
            self.selector_extractors.append(nn.Linear(256, 512, bias=False))
            self.value_extractors.append(nn.Sequential(nn.Linear(256, 512),
                                                       nn.LeakyReLU()))

        # 共享参数的部分网络
        self.shared_modules = [self.key_extractors, self.selector_extractors,
                               self.value_extractors, self.img_encoders, self.info_encoders]
    ...
    def forward(self, imgs, infos, acts):
        combines = [torch.unsqueeze(torch.cat((info, act), dim=1), 0) for info, act in zip(infos, acts)]
        ...
        all_encodings = [torch.cat((img_encoding, torch.squeeze(info_encoding)), dim=1) for img_encoding, info_encoding in
                         zip(img_encodings, info_encodings)]
        # img_encodings2 = [img_encoder2(img) for img, img_encoder2 in zip(imgs, self.img_encoders2)]
        # info_encodings2 = [info_encoder2(torch.unsqueeze(info, 0)) for info, info_encoder2 in
        #                    zip(infos, self.info_encoders2)]
        # s_encodings = [torch.cat((img_encoding2, info_encoding2), dim=1) for img_encoding2, info_encoding2 in
        #                zip(img_encodings2, info_encodings2)]
        ...
        all_head_selectors = [[sel_ext(enc) for enc in all_encodings]
        ...
        for i in range(self.agent_num):
            critic_in = torch.cat((all_encodings[i], *other_all_values[i]), dim=1)
            q = self.critics[i](critic_in)
            reg = 1e-3 * sum((logit ** 2).mean() for logit in all_attend_logits[i])
            qs.append(q)
            regs.append(reg)
        return qs, regs
