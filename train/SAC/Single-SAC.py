...
import torch
from train.SAC import SAC as SAC
...
# 清除tensorboard文件
for file in os.listdir('C:/MaCA/runs/SAC'):
    path = os.path.join('C:/MaCA/runs/SAC', file)
    os.remove(path)

if __name__ == "__main__":
    # 红色方为fix rule no attack，蓝色方为SAC
    red_agent = Agent()
    ...
    blue_fighter_model = SAC.RLFighter(name='blue', agent_num=(DETECTOR_NUM + FIGHTER_NUM) * 2, attack_num=ATTACK_IND_NUM,
        ...
        while True:
            ...
            if step_cnt % 50 == 0:  # 50个step内航向保持一致，只更新攻击类型
                ...
                for y in range(blue_fighter_num):  # 可以不用for循环，直接矩阵计算
                    ...
                    true_action = blue_fighter_model.net_policy.get_action(tmp_img_obs, tmp_info_obs, alive)
            ...
            if env.get_done():
                ...
                blue_fighter_model.learn('model/SAC/single', writer)
